...
void fatiguepredict(cv::Mat &frame, cv::CascadeClassifier &fcc, LBFRegressor &regressor, cv::HOGDescriptor &eyeHog, const int &eyeHogSize,
		...
		printf("face detection time:%f\t", faceDetectionTime);
		...
		if (!faces.empty())
		{
			...
			printf("face alighment time:%f\t", faceAlignmentTime);
			...
			printf("predict time:%f\t", double(end - begin)/ CLOCKS_PER_SEC);

			end = clock();
			float totaltime = double(end - tt1) / CLOCKS_PER_SEC;
